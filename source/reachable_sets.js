  ...
  displayGrid(graphics,currentState,sweptStateX,sweptStateY){
    ...
    for(let indexX = 0;indexX < reachableSet.gN[sweptStateX];indexX++){
      for(let indexY = 0;indexY < reachableSet.gN[sweptStateY];indexY++){
        ...
        let state = currenState.slice();
        state[sweptStateX] = this.indexToState(index[sweptStateX],sweptStateX);
        state[sweptStateY] = this.indexToState(index[sweptStateY],sweptStateY);
        let mappedState =
          graphics.mapper.mapStateToPosition(state[sweptStateX],state[sweptStateY]);
          // Choose the correct color
        if(valuation > 0){
          // this is the safe zone
          graphics.beginFill(0x0BD480);
        }
        else{
          ...
          // Draw the circle
        graphics.drawCircle(mappedState[0],mappedState[0],2);
        graphics.endFill();
      }
    }
    //
